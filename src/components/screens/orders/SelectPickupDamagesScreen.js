...
class SelectPickupDamagesScreen extends React.Component {
  ...
  renderParcel = (item) => {
    ...
    return (
      <TouchableDebounce key={`${Math.floor(Math.random() * 100000)}`} 
        onPress={() => this.onChangeParcelState(parcel.id)}>
  ...
  renderScannedParcels = () => {
    ...
    return <>
      ...
      {this.renderTabPane(parcels, parcel => true)}
    </>;
  }
...
export default connect(mapStateToProps, mapDispatchToProps)(SelectPickupDamagesScreen);
