...
import {sagaSubmitCollectionSuccess} from '../../../redux/sagas/route';
import {bindActionCreators} from 'redux';
import TouchableDebounce from '../common/TouchableDebounce';

class SenderDetailsScreen extends React.Component {
  ...
  constructor(props) {
    ...
    this.collect_place = this.props.navigation.state.params.collect_place;
  ...
  onSubmit = () => {
    ...
    if (!senderName) {
      Alert.alert(
        'Error',
        "Sender's name is required to finish the delivery.",
      );
      return;
    }
    if (!parcelsPicture) {
    ...
    if (!signature) {
      Alert.alert(
        'Error',
        "Sender's signature is required to finish this collection.",
      );
      return;
    }

    console.log('Stop ID AAA:', this.stop.id);

    this.props.submitCollectionSuccess(
      {
        sender_name: senderName,
        signature: signature,
        collect_place: this.collect_place,
        collection_picture: parcelsPicture,
          ...
          <ScrollView
            ...
            <View style={AppStyles.roundSection}>
              <View style={AppStyles.formGroup}>
                <Text style={AppStyles.label}>Name of the Sender</Text>
                <TextInput
                  style={[AppStyles.textInput]}
                  value={this.state.senderName}
                  autoCapitalize="words"
                  onChangeText={(val) => this.setState({senderName: val})}
                />
              </View>

              <View style={AppStyles.formGroup}>
                <Text style={AppStyles.label}>Picture of Collected Parcels</Text>
...
const mapDispatchToProps = (dispatch) =>
  bindActionCreators(
    {
      submitCollectionSuccess: sagaSubmitCollectionSuccess,
    },
    dispatch,
  );
