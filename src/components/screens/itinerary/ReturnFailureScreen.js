...
import {sagaSubmitReturnFailure} from '../../../redux/sagas/route';
import TouchableDebounce from '../common/TouchableDebounce';

class ReturnFailureScreen extends React.Component {
  ...
  onSubmit = () => {
    if (!this.state.parcelsPicture) {
      Alert.alert('Error', 'Please take a picture of the visited place.');
      return;
    }

    this.props.submitReturnFailure(
      {
          ...
          <ScrollView>
            {this.failure_reason ===
              ...
              <Text
                ...
                The delivery has been failed because the recipient is not
                available in the destination.
              </Text>
            )}
            {this.failure_reason === consts.DELIVERY_FAILURE_TYPES.SHOP_CLOSED && (
              <Text
                style={[
                  AppStyles.text,
                  {
                ...
                The delivery has been failed because the business was
                closed.
              </Text>
            )}
...
const mapDispatchToProps = (dispatch) =>
  bindActionCreators(
    {
      submitReturnFailure: sagaSubmitReturnFailure,
    },
    dispatch,
  );
