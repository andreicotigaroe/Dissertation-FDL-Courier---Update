...
class VerifyParcelScreen extends React.Component {
  static navigationOptions = ({ navigation }) => {
    const { params = {} } = navigation.state;
    return {
      ...HeaderStyle,
      title: 'VERIFY PARCELS',
      headerRight: () => {
        return (
  ...
  constructor(props) {
    ...
    var parcels = JSON.parse(JSON.stringify(this.order.parcels));
  ...
  addParcelHandler = () => {
    ...
    const parcel = parcels.find(parcel => parcel.id === inputParcelID.toUpperCase());
    if (parcel) {
      parcel.scanned = true;
  ...
  onNext = () => {
    ...
    this.props.navigation.replace("DeliveryStatus", {
      parcels: parcels,
      stop: this.stop,
      order: this.order,
    });
  ...
  onFinishScan = () => {
    ...
    if (scannedNum === parcelNum) {
      ...
      Alert.alert("Scan Confirmation", "Are you sure you scanned all the parcels? Not scanned parcels will be regarded as missing ones.", [{
        text: 'NO',
        style: "cancel",
        onPress: () => {
  ...
  render() {
    const { flashOn, parcels, foundParcel, foundWrongParcel, visibleParcelInputModal, inputParcelID } = this.state;
    ...
    stops.forEach((stop, index) => {
      ...
      for (var i = 0; i < stop.parcels.length; i++) {
        const parcel = this.state.parcels.find(_parcel => _parcel.id === stop.parcels[i].id);
        if (parcel) {
    ...
    return (
      <SafeAreaView style={{ flex: 1 }}>
        <Waiting visible={this.props.route.calling} title={"Submitting loading data..."} />
        ...
        <View style={{ height: 250, justifyContent: "center", overflow: "hidden", alignItems: "center" }}>
          <QRCodeScanner
            ...
            markerStyle={{ borderWidth: (foundParcel || foundWrongParcel) ? 0 : 1, width: 180, height: 180, marginTop: -40 }}
          ...
          {foundWrongParcel && <View style={{ position: "absolute", width: "100%", height: "100%", backgroundColor: "#ff000060", justifyContent: "center", alignItems: "center" }}>
            <Ionicon name="close-circle-outline" color={"#ffffff"} size={120} />
          </View>}
        </View>

        <View style={styles.bottomBar}>
          <View style={styles.bottomBarRow}>
            <Text style={styles.textSummary}>SCAN {this.state.parcels.length} PARCELS</Text>
        ...
        <View style={{ flex: 1 }}>
          <Text style={{ fontFamily: AppFonts.main.medium, width: "100%", fontSize: 16, textAlign: "center", paddingVertical: 10 }}>{scannedParcels.length} / {this.state.parcels.length} Parcels Scanned</Text>
          {this.renderScanned()}
        </View>
...
export default connect(mapStateToProps, mapDispatchToProps)(VerifyParcelScreen);
